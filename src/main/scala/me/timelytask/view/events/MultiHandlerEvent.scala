...
trait TypeSensitiveHandler[T, ArgsType, Args <: ArgWrapper[?, ArgsType]] {
 ...
 isPossibles: List[Args => Option[InputError]]) {
  def call[T](args: Args): Boolean = {
    if isPossibles.exists(isPossible => isPossible match
      ...
      case _ => false)
    then handlers.exists(handler => handler match {
...
trait MultiHandlerEventCompanion[ArgsType, Args <: ArgWrapper[?, ArgsType], EventType 
<: MultiHandlerEvent[ArgsType, Args]] {
  ...
  def addHandler(newHandler: TypeSensitiveHandler[?, ArgsType, Args], isPossible: Args => Option[InputError]): Unit = {
