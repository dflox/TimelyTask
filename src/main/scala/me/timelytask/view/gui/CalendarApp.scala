    ...
    stage = new PrimaryStage {
      ...
      scene = new Scene(800, 600) {
        ...
        for (i <- 1 to daysToShow) {
          ...
          if (date.withTimeAtStartOfDay() == today.withTimeAtStartOfDay()) {
            dateText.fill = Color.Red
          }
          val dateCell = new StackPane {
            style = if (date.withTimeAtStartOfDay() == today.withTimeAtStartOfDay()) "-fx-background-color: rgba(255, 0, 0, 0.1);" else ""
            children = dateText
          }
        ...
        val currentTimeLine: Line = new Line {
          startX = 0
          endX = 700
          startY = 0
          endY = 0
        ...
        val timeMarker: StackPane = new StackPane {
          style = "-fx-background-color: rgba(255, 0, 0, 0.1);"
          children = Seq(currentTimeLine)
          padding = Insets((currentTime.getHourOfDay * 50) + currentTime.getMinuteOfHour * 0.83, 0, 0, 0)
        }
