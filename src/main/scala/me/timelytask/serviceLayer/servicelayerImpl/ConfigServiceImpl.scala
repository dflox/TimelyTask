...
class ConfigServiceImpl(serviceModule: ServiceModule) extends ConfigService {
  ...
  override def updateConfig(userName: String, config: Config): Unit =
    serviceModule.updateService.updateConfig(userName, Config.default)
  ...
  override def addConfig(userName: String, config: Config): Unit = {
    serviceModule.updateService.updateConfig(userName, config)
  }
}
